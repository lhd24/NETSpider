...
namespace WinFormLib.Components.Cell
{
    ...
    public class DataGridViewAutoFilterColumnHeaderCell : DataGridViewColumnHeaderCell
    {
        ...
        private void VerifyDataSource()
        {
            ...
            IBindingGridListView data = this.DataGridView.DataSource as IBindingGridListView;
            if (data == null)
            {
        ...
        private void ResetFilter()
        {
            ...
            IBindingGridListView source = this.DataGridView.DataSource as IBindingGridListView;
            if (source == null || String.IsNullOrEmpty(source.Filter))
            {
                filtered = false;
                selectedFilterValue = "(All)";
                currentColumnFilter = String.Empty;
                source.ConditionExpression = null;
            }
        }
        ...
        protected override void OnMouseDown(DataGridViewCellMouseEventArgs e)
        {
            ...
            //    return;
            //}
            if (this.OwningColumn.Resizable == DataGridViewTriState.True && e.X < 6)
            {
                return;
            }
                ...
                e.X + cellBounds.Left - scrollingOffset, e.Y + cellBounds.Top))
            {
                ...
                if (this.DataGridView.IsCurrentCellInEditMode)
                {
                    ...
                    IBindingGridListView source =
                        this.DataGridView.DataSource as IBindingGridListView;
                    if (source != null)
                    {
                        source.EndEdit();
                    }
                ...
                else
                {
                    ...
                    foreach (PropertyInfo property in properties)
                    {
                            ...
                            System.Globalization.CultureInfo.InvariantCulture) == 0)
                        {
                            ...
                            var vau = this.OwningColumn.CellTemplate.FormattedValue;
        ...
        private Expression GetFinalExpression(IBindingGridListView data, string fieldName, object[] options)
        {
            ParameterExpression left = Expression.Parameter(data.DataSourceType, "c");//c=>
            Expression expression = Expression.Constant(false);
            foreach (var optionName in options)
            {
                Expression right = Expression.Call
                       (
                          Expression.Property(left, data.DataSourceType.GetProperty(fieldName)),  //c.DataSourceName
            ...
            Expression finalExpression = Expression.Lambda(expression, new ParameterExpression[] { left });
            return finalExpression;
        }
        private Expression GetFinalExpression(IBindingGridListView data, string fieldName, object value)
        {
            ParameterExpression xExpr = Expression.Parameter(data.DataSourceType, "x");

            PropertyInfo propertyInfo = data.DataSourceType.GetProperty(fieldName);
            Type propertyType = propertyInfo.PropertyType.GetUnderlyingType();

            value = Convert.ChangeType(value, propertyType);
            return Expression.MakeBinary(ExpressionType.Equal, Expression.Property(xExpr, propertyInfo), Expression.Constant(value, propertyInfo.PropertyType));
        }
        /// <summary>
        ...
        private void UpdateFilter()
        {
            ...
            // Cast the data source to an IBindingGridListView.
            IBindingGridListView data =
                this.DataGridView.DataSource as IBindingGridListView;

            Debug.Assert(data != null && data.SupportsFiltering,
                "DataSource is not an IBindingGridListView or does not support filtering");
            ...
            Expression newConditionExpression = null;
            ...
            switch (selectedFilterValue)
            {
                ...
                default:
                    ...
                    newConditionExpression = GetFinalExpression(data, columnProperty, selectedFilterValue);
                    break;
            }
            ...
            if (String.IsNullOrEmpty(newFilter))
            {
                ...
                data.ConditionExpression = Expression.Lambda(newConditionExpression, Expression.Parameter(data.DataSourceType, "x"));
            }
            else
            {
                newFilter += " AND " + newColumnFilter;
                data.ConditionExpression = Expression.AndAlso(data.ConditionExpression, newConditionExpression);
            }
        ...
        public static void RemoveFilter(DataGridView dataGridView)
        {
            ...
            // Cast the data source to a BindingSource.
            IBindingGridListView data = dataGridView.DataSource as IBindingGridListView;

            // Confirm that the data source is a BindingSource that 
            // supports filtering.
            if (data == null || !data.SupportsFiltering)
            {
        ...
        public static String GetFilterStatus(DataGridView dataGridView)
        {
            ...
            if (String.IsNullOrEmpty(data.Filter) || data == null || !data.SupportsFiltering)
            {
                return String.Empty;
            }
